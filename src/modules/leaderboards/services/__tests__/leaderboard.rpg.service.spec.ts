...
import { LeaderBoardsServiceRPG } from '../leaderboard.rpg.service';
...
describe('LeaderBoardsServiceRPG', () => {
  let service: LeaderBoardsServiceRPG;
  let rpgPrismaService: RpgPrismaService;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: RpgPrismaService,
          useValue: mockRpgPrismaService,
        },
        {
    ...
    service = module.get<LeaderBoardsServiceRPG>(LeaderBoardsServiceRPG);
    rpgPrismaService = module.get<RpgPrismaService>(RpgPrismaService);
  ...
  describe('getLeaderboardByType', () => {
    ...
    it('should return leaderboard data for KILLS type', async () => {
      ...
      expect(rpgPrismaService.rankingKill.findMany).toHaveBeenCalled();
    ...
    it('should return leaderboard data for DEATHS type', async () => {
      ...
      expect(rpgPrismaService.rankingDeath.findMany).toHaveBeenCalled();
      expect(result).toHaveLength(2);
    });

    it('should return leaderboard data for KD type', async () => {
      const result = await service.getLeaderboardByType(StatsType.KD, options);

      expect(rpgPrismaService.rankingKD.findMany).toHaveBeenCalled();
      expect(result).toHaveLength(2);
    });

    it('should return leaderboard data for LEVEL type', async () => {
      const result = await service.getLeaderboardByType(
        StatsType.LEVEL,
        options,
      );

      expect(rpgPrismaService.rankingLevel.findMany).toHaveBeenCalled();
      expect(result).toHaveLength(2);
    });
    ...
    it('should return leaderboard data for MAX_STREAK type', async () => {
      ...
      expect(rpgPrismaService.rankingMaxStreak.findMany).toHaveBeenCalled();
      expect(result).toHaveLength(2);
    });

    it('should return leaderboard data for ELO type', async () => {
      const result = await service.getLeaderboardByType(StatsType.ELO, options);

      expect(rpgPrismaService.rankingElo.findMany).toHaveBeenCalled();
      expect(result).toHaveLength(2);
    });
    ...
    it('should return leaderboard data for KOTH type', async () => {
      ...
      expect(rpgPrismaService.rankingKoth.findMany).toHaveBeenCalled();
      expect(result).toHaveLength(2);
    });

    it('should throw InvalidLeaderboardTypeException for invalid type', async () => {
      const invalidType = 'invalid' as StatsType;
    ...
    it('should pass through InvalidLeaderboardTypeException', async () => {
      jest.spyOn(service, 'fetchLeaderboardByType').mockImplementation(() => {
        throw new InvalidLeaderboardTypeException(StatsType.KILLS);
      });

      await expect(
        service.getLeaderboardByType(StatsType.KILLS),
      ).rejects.toThrow(InvalidLeaderboardTypeException);
    });
  ...
  describe('fetchLeaderboardByType', () => {
    ...
    it('should use default values for limit and offset if not provided', async () => {
      await service.fetchLeaderboardByType(StatsType.KILLS, {});

      expect(rpgPrismaService.rankingKill.findMany).toHaveBeenCalledWith(

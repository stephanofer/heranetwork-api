...
import { LeaderboardsControllerRPG } from '../leaderboards.rpg.controller';
...
describe('LeaderboardsControllerRPG', () => {
  let controller: LeaderboardsControllerRPG;
  let leaderBoardsService: LeaderBoardsServiceRPG;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      controllers: [LeaderboardsControllerRPG],
      providers: [
        {
          provide: LeaderBoardsServiceRPG,
    ...
    controller = module.get<LeaderboardsControllerRPG>(
      LeaderboardsControllerRPG,
    );
    leaderBoardsService = module.get<LeaderBoardsServiceRPG>(
  ...
  describe('getLeaderboard', () => {
    const query: GetLeaderboardDto = {
      type: StatsType.KILLS,
      limit: 10,
      offset: 0,
    };
    ...
    it('should return cached data if available', async () => {
      ...
      expect(cacheManager.get).toHaveBeenCalledWith(
        `LeaderBoardRPG:${query.type}:limit=${query.limit}:offset=${query.offset}`,
      );
    ...
    it('should use default values for limit and offset if not provided', async () => {
      const partialQuery: Partial<GetLeaderboardDto> = {
        type: StatsType.KILLS,
      };
      cacheManager.get.mockResolvedValueOnce(null);

      await controller.getLeaderboard(
        partialQuery as GetLeaderboardDto,
        mockResponseObject as any,
      );

      expect(cacheManager.get).toHaveBeenCalledWith(
        `LeaderBoardRPG:${partialQuery.type}:limit=150:offset=0`,
      );
      expect(leaderBoardsService.getLeaderboardByType).toHaveBeenCalledWith(
        partialQuery.type,
        { limit: 150, offset: 0 },
